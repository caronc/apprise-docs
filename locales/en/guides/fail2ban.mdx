...
:::note[Detected IP]
For your convenience, this guide detected your public address as <code><MyIPAddr prefix={true} client:load /></code>. If this is the workstation you will remote into your server from, you may wish to add this to the `ignore=` line.
:::
...
#
action = apprise[config="/etc/fail2ban/apprise.conf", apprise_args="--tag fail2ban"]
...
```ini
action = apprise[config="http://api.example.ca:8000/cfg/apprise-key", apprise_args="--tag fail2ban"]
```
