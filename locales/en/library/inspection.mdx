...
| `type`     | Value type. Defaults to `string`; all supported forms are listed below. |
...
The `type` field tells clients how to collect and encode a value:

| Type            | Meaning                                                                                            |
| --------------- | -------------------------------------------------------------------------------------------------- |
| `string`        | One text value.                                                                                    |
| `email`         | One email address. Preserve its `@` when building the URL.                                         |
| `bool`          | One true/false value; must include a `default`.                                                    |
| `int`           | One integer, optionally bounded by `min` and `max`.                                                |
| `float`         | One decimal number, optionally bounded by `min` and `max`.                                         |
| `choice:<type>` | One entry from `values`. The item type may be `string`, `int`, `float`, or `email`.                |
| `list:<type>`   | Multiple items separated using `delim`. Items may be `string`, `bool`, `int`, `float`, or `email`. |

Use `bool` rather than `choice:bool`.
...
Every service exposes an `args.format` entry (`text`, `html`, `markdown`). `values` is always all three — `?format=` is accepted for any of them on every service — and `default` is the service's native format.

Since some services natively render more than one format, `args.format` also carries a `supported` list: the subset this specific service actually renders as, in priority order (`supported[0]` matches `default`). For most services `supported` is a single entry equal to `default`. Requesting a `?format=` value that isn't in `supported` still works — the service logs a warning and falls back to its default rather than rejecting the URL.

Use `supported` when you want to show every format option but de-emphasize (not remove) the ones a service doesn't specifically support — `values` stays the full, always-safe set to fall back on if you don't need that distinction.
...
6. Respect `type` to render correct widgets:
   - `string`: text input
   - `bool`: checkbox
   - `email`: email input; preserve `@` when building the URL
