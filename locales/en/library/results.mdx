...
The earlier examples read logs after `notify()` finishes. Use a callback when a
console or progress view must show warnings and errors while services are still
running.

Use `log_callback` to receive each `NotifyLogEntry` as it happens. Apprise
calls it synchronously with `(entry, service)`. A function, lambda, or other
callable can be used.

<Tabs>
<TabItem label="Synchronous">

Print, collect, or forward each entry without `asyncio`:
...
# Runs as each service warning or error occurs.
apobj.notify(body="Deploying new release")
```

</TabItem>
<TabItem label="Async Work">

`log_callback` cannot be `async def`, even in an `asyncio` app. Schedule async
work from a regular callback instead. Keep each task referenced until it
finishes, and check it for errors:

```python
# Keep in-flight sends referenced until they finish.
background_tasks = set()
loop = asyncio.get_running_loop()

def log_task_error(task):
    # Report failures without reading a cancelled task's exception.
    if not task.cancelled() and (exc := task.exception()):
        print(f"publish_log failed: {exc}")

def schedule_log(entry, service):
    # This function runs safely on the event loop.
...
Set a default with `Apprise(log_callback=...)`, or pass `log_callback` to
`notify()` or `async_notify()` for one send:
...
- **It's optional.** It does not change `notify()` results or stored logs.
- **Set it once, or just for one call.** `Apprise(log_callback=...)` applies to
  every send from that object. Passing it to `notify()` or `async_notify()`
  overrides the default for one call:
...
- **It may be called from more than one thread at once.** Apprise can notify
  services in parallel. Keep the callback short and protect shared state; use
  `queue.Queue`, for example, instead of changing a shared list.
- **It must be a plain synchronous function, not `async def`.** It's called
  inline and should return quickly. Schedule async work as shown above. Apprise
  closes an accidentally returned coroutine and logs a warning.
...
| I want to see logs live, as they happen           | `log_callback=` on `Apprise()`, `notify()`, or `async_notify()` |
