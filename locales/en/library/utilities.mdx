...
- `is_secure_url(url)`
  Validates a URL that was handed to a plugin by a remote server, such as a
  service discovery record or a `.well-known` file. It returns `True` only when
  the URL uses HTTPS, names a valid host, carries no username, password, or
  non-empty fragment, and has a usable port. Private and loopback addresses
  stay valid so self-hosted services keep working.

- `is_uuid(value)`
  Validates UUID strings.

Apart from `is_secure_url()`, which answers `True` or `False`, these helpers
return `False` when invalid and a normalized string (not a boolean) when
valid.
