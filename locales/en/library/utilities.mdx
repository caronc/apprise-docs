...
- `is_secure_http_url(url)`
  Checks a URL received from a remote server, such as through service discovery
  or a `.well-known` file. It returns `True` only for HTTPS URLs with a valid
  host and port, and without a username, password, or fragment. Private and
  loopback addresses remain valid for self-hosted services.

- `is_uuid(value)`
  Validates UUID strings.

Apart from `is_secure_http_url()`, which returns `True` or `False`, these
helpers return `False` for invalid input and a normalized string for valid
input.
