...
names as `${NAME}` in its URLs or settings. See
[Template Variables](../getting-started/template/) for
the YAML syntax and placement guidance.
...
call or environment. Treat returned defaults as configuration content.
...
Blank or whitespace-only call and environment values are ignored. An explicit
empty-string default (`api_key: ""`) is still a value, so it takes priority over
`APPRISE_TEMPLATE_API_KEY`.
...
`UserDict` works alongside a plain `dict`. Invalid input raises
`apprise.exception.AppriseTemplateError` from `find()`; `notify()` returns
`FAILURE`.

An entry with no available value is skipped. The result is `PARTIAL` when
another matching entry sends, or `FAILURE` when every matching entry is skipped.

`find(..., resolve=False)` returns pending entries without filling them in.
`urls()` and iteration do the same.

One case-insensitive set of values covers everything in an `Apprise` object.
Extra valid names are ignored and recorded by name in the local debug log.
...
This ignores `template:` sections and environment values, and treats `${NAME}`
as ordinary text. Only the application can change this setting.

Per-call values do not change the cached configuration or later calls. Reusing
the same values also reuses the built service and storage identity.

Missing variable names are written to the local log, but not to
`AppriseResult` or `log_callback` output.
