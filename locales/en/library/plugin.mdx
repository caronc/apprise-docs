...
<TabItem value="targets" label="Multiple targets">
This pattern is ideal for:

- Sending the same message to several rooms, numbers, or addresses
- Any service where one target can fail while the others succeed
...
class NotifyDemoTargets(NotifyBase):
    ...
    def send(self, body, title="", notify_type=NotifyType.INFO, **kwargs):
        ...
        for target in self.targets:
            # This target already got the message on an earlier attempt
            ...
            # Record it so a retry skips this target
...
class NotifyMyService(NotifyBase):
    ...
    def send(
        ...
        **kwargs: Any,
        ...
        # TODO: Replace with a real implementation. For multiple targets,
        # use is_delivered()/mark_delivered() as shown above.
        self.logger.debug(
...
### Throttling, Verification, Redirects, and Timeouts

`URLBase` defines common SSL verification, redirect behaviour, and socket
timeouts as URL arguments. Always pass the corresponding attributes into every
`requests.*` call your plugin makes:

- `verify` controls `verify_certificate` -- pass as `verify=self.verify_certificate`
- `redirect` controls `redirects` -- pass as `allow_redirects=self.redirects`
...
## Delivering to More Than One Target

If `send()` loops over several targets, record each successful target. A
retry then contacts only the targets that failed.
...
`is_delivered()` checks whether a target already received the message.
Call `mark_delivered()` only after the target accepts it.

The key identifies the target, such as a phone number, room ID, token, or
email address. Lists and dictionaries also work.

Notes:

- Both methods do nothing when `retry=0`, which is the default.
- Each piece of a split message is tracked on its own.
- Tracking is dropped when the notification finishes.
- Something sent once for the whole notification, such as an image, uses
  `per_message=True` so it is not repeated for every piece:
...
- For batches, use a stable key such as `(category, batch_position)` so each
  batch remains distinct.
- If one target needs several requests, include the request position in the
  key and mark each request only after it succeeds.
- If you count successes, count a skipped target too:
...
Only sending loops need this tracking. Loops that build a payload, split a
message, or collect recipients for one request do not.

To share other values between attempts, see **Carrying State Across Retries** below.

## Carrying State Across Retries

`remember()` and `recall()` share a value across one notification's attempts. The value is discarded when the notification ends, so the next notification starts clean.

Use them for values a retry should reuse, such as a request ID, upload handle, or temporary token.

| Situation            | Behaviour                                                                                      |
| -------------------- | ---------------------------------------------------------------------------------------------- |
| Retries are enabled  | Every attempt of this notification reads back what an earlier one remembered.                  |
| Retries are disabled | `remember()` keeps nothing and `recall()` returns its default. No later attempt would read it. |

:::note
Do not use `self.store` for retry-only state. It outlives a notification and requires an arbitrary expiry, while `remember()` is cleared automatically when the notification ends.
:::

<Tabs>
<TabItem label="Idempotency key">

An idempotency key prevents a service from acting twice. Reuse the original key when retrying a request that never returned.
...
# One notification can send several requests, so name this one too
...
<TabItem label="Skipping Repeated Work">

Anything expensive a retry should not redo, such as uploading a file before the message that references it.

```python
def _media_id(self, attachment):
    """Upload once per notification rather than once per attempt."""

    # per_message, because one upload serves every piece of a
    # split message
...
<TabItem label="With Delivery Tracking">

Use `remember()` and `recall()` for shared retry state. Use `is_delivered()` and `mark_delivered()` to track which targets already accepted the notification.

```python
def send(self, body, title="", notify_type=NotifyType.INFO,
         attach=None, **kwargs):
    """Upload once, then reach every target that is still waiting."""

    has_error = False

    # Uploaded on the first attempt and reused by the rest
    media_id = self._media_id(attach) if attach else None

    for target in self.targets:
        # Accepted on an earlier attempt, so leave it alone
        if self.is_delivered(target):
            continue
...
- Both methods are inert when retries are disabled, so the default `retry=0` path costs nothing and needs no guard of your own.
- Each split piece keeps its own value, even when its content matches another piece. Set `per_message=True` only for state shared by the whole notification, such as one uploaded file used by every piece.
- When one notification makes several requests, use a distinct key for each request. A hash of the request body usually works well.
- Key types stay distinct, so `1`, `"1"` and `["1"]` do not share state.
...
### `runtime_deps()` — Memory Eviction Hint

If your plugin **conditionally imports a heavy third-party library**, also override `runtime_deps()` to tell the plugin manager which top-level package names it loaded.
...
The [Notification Manager](./extending/notification-manager/#library-eviction) tracks a reference count for each declared library. When every plugin that uses a given library is disabled and `N_MGR.evict_on_disable` is `True`, the manager removes that library — and all of its submodules — from `sys.modules`, releasing the associated Python objects from memory.
...
- This is separate from `requirements`, which names pip packages for human-readable dependency reporting. `runtime_deps()` names importable packages for runtime memory management.

:::note
`runtime_deps()` is only useful when the library is loaded conditionally (imported only if the plugin is active). If your plugin always imports the library at module load time, eviction has no practical effect because the module is already resident.
:::
...
| `redirects`              | `True`                           | HTTP redirect following, used when `redirect` is not provided. Falls back to `asset.http_redirects`. | Set to `False` on the asset to disable redirects globally if your environment requires it.                            |
...
| `template_args`          | `{ verify, redirect, rto, cto }` | Common URL args and their defaults.                                                                  | Usually extend rather than replace.                                                                                   |
...
| `notify_format`                     | `text`     | Default message format(s); one value or an ordered tuple/list. | If your service is Markdown or HTML-first, or natively renders more than one format. |
...
Apprise validates types using a strict pattern: `((choice|list):)?(string|bool|int|float|email)`.
...
| `email`         | tokens, args | A single email address       | none                | Use when `url()` preserves the address's `@` character.                               |
...
| `bool`          | tokens, args | A boolean flag               | `default`           | Boolean entries must provide a default.                                               |
...
| `choice:email`  | tokens, args | One email from a fixed set   | `values`            | Preserve the selected address's `@` character.                                        |
| `list:string`   | tokens, args | A list of strings            | `delim`             | List entries must provide delimiters.                                                 |
| `list:bool`     | tokens, args | A list of boolean values     | `delim`             | Use `bool` for a single boolean value.                                                |
...
| `list:email`    | tokens, args | A list of email addresses    | `delim`             | Preserve each address's `@` character.                                                |
...
### alias_of Inheritance Rule

Alias entries must **never** redeclare properties that are inherited from the target. The following keys are forbidden on any entry that contains `alias_of`:

`private`, `type`, `required`, `regex`, `default`, `min`, `max`, `map_to`, `prefix`, `group`

These are resolved at runtime by following the alias chain — consumers such as the [URL Builder](../url-builder/) read them from the target, not from the alias itself. The only permitted extras alongside `alias_of` are:

| Extra key | When allowed                                                                                 |
| :-------- | :------------------------------------------------------------------------------------------- |
| `name`    | Required when `alias_of` lists **multiple** targets, to provide a grouping label for the UI. |
| `delim`   | Allowed when the aliased target is a `list:*` type, to override the delimiter character(s).  |

Valid approaches:

1. Single alias — only alias_of

   ```python
   "t": {"alias_of": "token"},
   ```

1. Multiple aliases — alias_of + name for grouping label
...
1. List alias with delimiter override
...
| :--------- | :------------------- | :-------------------------------------------------------------------------------------------------- |
| `name`     | tokens, args, kwargs | Human-friendly label, usually wrapped in `gettext_lazy()`.                                          |
| `type`     | tokens, args         | Value type: `string`, `bool`, `int`, `float`, `email`, `choice:<type>`, or `list:<type>`.           |
...
Each `template_kwargs` entry requires a `prefix` and a `name`, and those are also surfaced in the [URL Builder](../url-builder/) as a **Custom Parameters** table where users can add any number of key/value rows.
...
| :----------------------------------- | :---------------------------------------------------------------------------------------------------------------------------- |
| `service_name`                       | Shown as the service display name in search results.                                                                          |
| `service_url`                        | Powers the "Service website" link next to the form.                                                                           |
| `setup_url`                          | Powers the "Setup guide" link for step-by-step instructions.                                                                  |
...
Mark every API key, token, password, or secret with `"private": True` in both `template_tokens` and `template_args`. The URL Builder:
...
Alias entries (`alias_of`) **inherit** the `private` flag from their target — do not redeclare it on the alias.

### service_url vs setup_url

| Attribute     | Purpose                                                                                                                                                                                                                                                                     |
| :------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `service_url` | Links to the upstream vendor or product landing page (e.g. `https://discord.com/`). Shown in the URL Builder as "Service website".                                                                                                                                          |
| `setup_url`   | Links to the Apprise-specific integration guide on `appriseit.com` (e.g. `https://appriseit.com/services/discord/`). Shown as "Setup guide". You can create or update this documentation in the [Apprise documentation repository](https://github.com/caronc/apprise-docs). |

Both should always be set. Generic or local-only plugins (e.g. MQTT, Windows notifications) may omit `service_url` if there is no applicable upstream URL.

## Format and Overflow

`NotifyBase` supplies shared URL options such as `format`, `overflow`, `emojis`, and persistent storage (`store`). A plugin should normally reuse these options instead of defining its own versions.

During delivery, `body_format` is the resolved format the plugin must send. `body_passthrough=True` means the caller did not declare a source format, so Apprise skipped source-format conversion and dialect repair. It does not disable message limits, overflow handling, escaping, transport encoding, or payload assembly.

The framework handles ordinary character limits and the `upstream`, `truncate`, and `split` modes. A plugin with byte-based limits, escaping, multiple body representations, or encryption overhead should calculate its final budget from the payload it will actually send and verify the result after conversion.

Do not assume arbitrary pass-through content can be split safely. JSON documents, ciphertext, signatures, and unknown markup may become invalid when divided. Honour the requested overflow mode, but treat splitting as best effort and avoid format-specific repair when `body_passthrough` is true.
