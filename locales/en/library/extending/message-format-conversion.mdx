...
By the time `_build_send_calls()` and `send()` run, conversion and sizing are already done. What your plugin receives:

- **`body`** — already converted to `self.notify_format` and sized according to `overflow_mode`. With `SPLIT`, each `send()` call receives one chunk of at most `body_maxlen` characters. With `TRUNCATE`, the body is capped at `body_maxlen`. With `UPSTREAM` (the default), the body arrives unmodified and may exceed the declared limit. Do not call `convert_between()`, `html_to_markdown()`, or re-split the body in `send()`.
...
When calling `super()._build_send_calls()`, pass the format of the adapted body so the base class selects the right splitting strategy for the content it receives:
...
Passing the wrong format causes character-boundary splits that break links, code spans, or other inline syntax. When translation happens inside `send()` rather than before the split, make sure each chunk's `body_format` still carries the original source format so `send()` can convert it correctly.

Telegram carries entity state between chunks and gracefully degrades spans that cannot fit within the upstream size limit; consult `NotifyTelegram._repair_split_chunk()` for a reference implementation.
...
2. **Do not re-convert the body** in `_build_send_calls()` or `send()`. It is already in `self.notify_format`. Use `body_format` to check what the content originally was, not to trigger another conversion pass.
...
6. **Set `overflow_mode`, `body_maxlen`, and `title_maxlen` on the class** to declare the limits and splitting strategy for your upstream service. Each `send()` call receives content already sized to those limits — do not re-split inside `send()`. Pass the adapted body's format to `super()._build_send_calls()` so the base class uses the right strategy.
