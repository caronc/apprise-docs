...
| `body_passthrough` | No       | `True` if no source was declared and automatic format conversion was skipped; otherwise `False`                                           |
...
By default, your function receives the body without automatic format conversion.
To have Apprise convert it first, add `body_format=` to `@notify()`:
...
`body_format=` accepts one format or a tuple of supported formats. The first
format in the tuple is the default. See
[Message Format Conversion](../message-format-conversion/) for more examples.

Without `body_format=`, Apprise does not convert the body. Your function still
receives `body_format` and `body_passthrough` in `kwargs`. `body_format` reports
the caller's format, or `TEXT` when the caller did not provide one.

Pass-through does not bypass the normal delivery pipeline. The body may already
have been split or truncated according to the configured overflow mode, and the
wrapper may still need to escape or package it safely for its destination. Use
`body_passthrough` to avoid making source-format assumptions.
