...
A long message may be split across several `send()` calls. Apprise includes
`attach` in only one of those calls, so `if attach:` prevents the same files
from being sent with every piece.

The attachment is sent with the first piece by default. To send it with another
piece, override `_attachment_send_index()`. See
[Knowing Your Position Among Multiple `send()` Calls](../extending/message-format-conversion/#knowing-your-position-among-multiple-send-calls)
for an example.
...
### Base64 When the Service Requires It
