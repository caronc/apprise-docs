...
The `optional` flag marks a service as a "nice to have" endpoint. When `optional=yes` is
...
This is useful any time you want to send a notification _opportunistically_: deliver it if
the endpoint is up, but do not let its absence signal a problem to the calling application.
