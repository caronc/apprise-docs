...
:::note
As of 2023, X (formerly Twitter) deprecated the v1.1 API for free-tier accounts. Apprise now uses the X API v2. The two modes have different subscription requirements:

- **Tweets** (`mode=tweet`) -- available on the **Free** X API tier and above.
- **Direct Messages** (`mode=dm`, the default) -- require a **Basic** or higher paid X API subscription. Free-tier accounts will receive a 403 error when attempting to send DMs.

Media uploads (attachments) still use the v1.1 media endpoint, which remains permitted on all access levels including Free.
:::
...
2. Next, grant the appropriate access permissions so that you can post or send DMs. After clicking on the app name under **Projects & Apps** (on left menu), click on **Set up** under the **User authentication settings** section.<br/>![X User authentication set up](./images/X-User-authentication-set-up.png)<br/>On the **User authentication settings** page, set the following
   - **App permissions**\
     Select **Read and write** if you want to post only. If you want to send DMs (requires Basic+ subscription), select **Read and write and Direct message**.
   - **Type of App**\
...
:::caution
Direct Messaging requires a **Basic** or higher paid X API subscription. If you are on a free developer account, use `mode=tweet` instead.
:::
...
A public tweet can be referenced like so (available on the Free X API tier and above):
...
| Mode           | No       | The X mode you want to operate in. Use `tweet` to post publicly (Free tier+) or `dm` to send a Direct Message (Basic+ subscription required). By default this is set to `dm`. |
...
Send a public tweet (works on Free X API tier):
...
Send a X DM to @testaccount (requires Basic+ X API subscription):
