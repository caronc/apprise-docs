...
| album     | No       | Groups photos and videos into Telegram albums when set to `yes`. GIFs, audio, and documents are sent separately in their original order. If Telegram refuses an album, each item is sent on its own instead. A photo or video that is still refused is then sent as a regular file. The default is `no`.                                                                                                                                                                                                                                                                                                |
...
:::note
Telegram accepts attachments up to 50 MB, and photos up to 10 MB. A larger photo is sent as a regular file instead. A file over 50 MB can't be sent, and the notification is reported as failed.
:::
...
Got several photos to send? Group them into a single Telegram album:

```bash
# Assuming our {bot_token} is 123456789:abcdefg_hijklmnop
# Assuming the {chat_id} belonging to lead2gold is 12315544
# The photos arrive as one album instead of three separate messages
apprise -vv -t "Test Message Title" -b "Test Message Body" \
   --attach=/path/to/one.jpg \
   --attach=/path/to/two.jpg \
   --attach=/path/to/three.jpg \
