...
| image          | No       | Whether to include a thumbnail image alongside the message body (default `yes`).                                                                                                                                                                                       |
| ignore_expired | No       | Ignore newly reported expired subscriptions when another target succeeds (default `yes`). See [Subscriptions That Stop Working](#subscriptions-that-stop-working).                                                                                                     |
...
People uninstall apps, clear their browser data, and replace phones. When that happens the push service tells Apprise the subscription no longer exists, so repeatedly trying that endpoint will not help. What Apprise does next depends on whether it is allowed to change your file:
...
- **The file is read-only, or it is remote.** Apprise leaves it alone and remembers the endpoint in persistent storage. Later sends skip it. The retired-endpoint list is cleared 30 days after its last update; without persistent storage, it does not survive a restart.
...
A dead subscription is **not** counted as a failed notification when a push service accepts the message for at least one other device. A retry cannot revive the dead subscription and may send another copy to healthy subscribers. To report the expiration as a failure, set `?ignore_expired=no`; when `?retry=` is also set, each retry resends to the remaining healthy subscribers.

This setting applies when the push service reports the expiration during the current send. Endpoints already recorded as retired are skipped; they cause a failure only when no other target succeeds.

If no device at all could be notified, the notification is reported as a failure whichever way this is set.

:::tip
When a browser issues a new endpoint, Apprise treats it as a new subscription.
The retired-endpoint list is also cleared after 30 days without an update.
:::
