...
| format              | No       | Message format: **text**, **html**, or **markdown**. Default is **text**.                                                                                                                                                                                                                                                                                                                         |
...
| e2ee                | No       | Controls Matrix Olm/MegOLM encryption. Default is **yes**. Apprise encrypts messages and attachments for encrypted rooms and creates new rooms with encryption enabled. Requires the `cryptography` package and **matrixs://**; webhook mode is not supported. When available, E2EE overhead is included in message sizing. Set to **no** to always send unencrypted and skip E2EE room creation. |
