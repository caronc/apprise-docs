...
- `matrixs://{user}:{token}@{hostname}?mode=hookshot&webhook_path=/webhook`
...
- `matrixs://{user}:{token}@{hostname}?mode=hookshot&webhook_path=/public-hooks`
...
| webhook_path        | No       | Used with **hookshot** mode to define the public webhook path. Defaults to **/webhook**. For example, if your hookshot instance is exposed at `https://hookshot.example/public-hooks/{token}`, then set `?mode=hookshot&webhook_path=/public-hooks`.                                                                                                                                                                                                                                                                                                                                                                                             |
...
```bash
apprise -vv -t "Test Message Title" -b "Test Message Body" \
   "matrixs://apprise:ABCDEFG12345@hookshot.example.com?mode=hookshot&webhook_path=/public-hooks"
```
