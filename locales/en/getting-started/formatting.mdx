...
| [Apprise-API](../../api/)    | `none` - content passed as is unless `format` is provided in the `/notify/` payload, or the server operator sets `APPRISE_DEFAULT_FORMAT` to change this default |

Formatting on the Apprise-API is entirely opt-in per request. An operator who wants every request on their server to assume a format without every caller specifying one can set the `APPRISE_DEFAULT_FORMAT` environment variable (e.g. `APPRISE_DEFAULT_FORMAT=text`) -- see the [Environment Variables reference](../../api/reference/environment/) for details. A `format` value in the request always takes priority over this server-wide default.
