...
[Apprise-Go](https://github.com/unraid/apprise-go) is a Go port of Apprise
maintained by the Unraid team. It is designed for environments where a small,
self-contained native binary is preferred over a Python runtime.

Apprise-Go follows Apprise's URL-based notification model, but it is a separate
implementation and may not match Python Apprise behavior exactly for every
service or edge case.

The documentation on this site primarily describes the Python Apprise library,
Apprise CLI, and Apprise API server. If you run into functionality issues
specific to Apprise-Go, please report them to the Unraid team through the
Apprise-Go project.
