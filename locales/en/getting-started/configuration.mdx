...
YAML sub-keys are merged with the values parsed from the URL before the service is created. This means common URL parts such as `user`, `password`, `host`, `port`, `verify`, and `tag` can be supplied as YAML keys, along with any service-specific parameter listed for that plugin.

All parameters listed in a service's `## Parameter Breakdown` table can be used as YAML sub-keys. The [email service](/services/email/) is shown above, but the same approach works for any Apprise service.

You can still keep values in the URL when that is more convenient. Query-string parameters in the URL, such as `?user=abc&password=123`, are parsed as URL overrides for the fields they represent.

The same pattern is useful for other services too. For example, an XMPP configuration can keep the URL short while placing credentials and connection behavior in YAML:
