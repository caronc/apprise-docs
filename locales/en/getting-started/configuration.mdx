...
YAML sub-keys always take precedence over values parsed from the URL. When the same field is defined in both the URL and as a YAML sub-key, the sub-key wins — this applies equally to structural URL components (`user:pass@host`) and any query-string parameters already embedded in the URL (`?user=abc&password=123`). Every common URL component — `user`, `password`, `host`, `port`, and `verify` — as well as every service-specific parameter from a plugin's `## Parameter Breakdown` table can be supplied as a sub-key.
