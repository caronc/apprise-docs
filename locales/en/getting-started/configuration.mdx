...
default. A name written with no value must be supplied**, and any URL using
it is skipped until it is.
...
2. An environment variable named `APPRISE_TEMPLATE_` plus the name in
...
An empty value counts as an answer at every one of these steps. Setting
`APPRISE_TEMPLATE_API_KEY=` deliberately leaves `${API_KEY}` blank rather than
falling back to the default written in the file.
