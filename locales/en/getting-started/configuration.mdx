...
Use the `_` placeholder when the URL itself should carry no embedded credentials:
...
:::note
In YAML token blocks always write `password:` -- not `pass:`. The `pass` shorthand is a URL-path convention (`user:pass@host`) resolved by the URL parser when it processes the URL string. YAML token keys map directly to plugin parameter names, where the correct name is `password`.
:::
