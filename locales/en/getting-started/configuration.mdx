...
### Two Equivalent Token Indentation Styles

Token sub-keys may be written at either the same indentation level as the URL key (**sibling form**) or indented one level deeper (**child form**). Both forms produce identical results.

**Child form** — tokens indented one level under the URL key (YAML standard nesting):
...
**Sibling form** — tokens at the same indentation level as the URL key:
...
Both produce exactly the same loaded service. The sibling form is especially convenient when copying configuration from tools or templates that use flat indentation.

The `_` placeholder is useful when the URL should carry no embedded credentials at all — every parameter is supplied through tokens:
...
:::note
In YAML token blocks, always write `password:` — not `pass:`. The `pass` shorthand is a URL-path convention (`user:pass@host`) that is resolved by the URL parser when processing the URL string itself. YAML token keys map directly to plugin parameter names, where the correct name is `password`.
:::
...
A YAML list under a URL creates one service definition for each list item. Apprise starts with the same parsed URL each time, then applies that item's YAML values as overrides.

This is useful when several destinations share the same credentials but need different targets, tags, or options:
...
The example above loads two `mailto://` services. Both inherit `user:pass@gmail.com` from the URL, while each entry gets its own `to` and `tag` value.
