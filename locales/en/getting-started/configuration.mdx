...
One of the most practical advantages of YAML over TEXT is that **parameter values never need to be URL-encoded**. In a plain URL, any value that contains special characters — spaces, `@`, `#`, angle brackets — must be percent-encoded before it can appear in a query string. In YAML, those values are plain strings with no encoding required.
...
:::note
The `schema://user:pass@host` portion of the URL is still standard URL syntax, so credentials embedded there still need URL-encoding when they contain reserved characters. Only the **sub-keys** (parameters moved out of the query string) are free of that requirement.
:::
