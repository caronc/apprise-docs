...
A value sent this way wins over anything set in the server's environment or
written as a default in the configuration. Names are case-insensitive. Extra
valid names are accepted and ignored, with their names acknowledged only in
the server's local debug log.

If a URL is still missing a value it is skipped, the rest are still notified,
and the response reports that not everything was sent. For safety the
response never says **which** value was missing; look in the server log for
that. Use the **Review** tab, or `GET /json/urls/{KEY}`, to see which names a
configuration is waiting for.

Values sent this way only affect a saved configuration. A stateless request may
include them, but they are unused and its URLs remain exactly as written.
...
the server fallback. You may remove rows freely, and **Clear Form** rebuilds the
suggestions. **Add Value** is available after every current row has a name;
highlighted rows will be sent. Quick tests use a separate concealed prompt.
Stateless tests are unchanged.

:::caution
URL variables are intentionally unrestricted and can change any field, including
the host. The completed destination receives that URL's credentials, path,
headers, payload fields, and settings. Every access level may supply declared
values, so use a named YAML setting when a caller should control only one option.
This is the safer choice for `public`, `locked`, or otherwise less-trusted
callers. A hard-to-guess Config ID is useful, but is not a security boundary.

A variable cannot replace the service name before `://`, or a `tag:`/`tags:`
value. The service and tags are selected before values are applied.
:::
...
See [Template Variables](/getting-started/configuration/#template-variables)
for how to set a configuration up.
