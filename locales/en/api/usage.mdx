...
server-wide default is `APPRISE_WORKER_COUNT * 4`. Additional streams receive
HTTP `200` and remain connected while queued; delivery starts automatically
when an active worker finishes. Work continues to occupy its slot if a client
disconnects during delivery. A reverse proxy may still enforce its own
connection or request limits before a request reaches Apprise API.
