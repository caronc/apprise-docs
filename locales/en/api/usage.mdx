...
- `null` means there is no default to hand over. Either none was written, or `privacy=1` withheld it.
- `null` does not mean the value is unavailable. The server may still fill the name in from its own environment when the notification is sent.
