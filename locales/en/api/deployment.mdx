...
`APPRISE_BASIC_AUTH_REALM` controls the name shown in login prompts. It does not change the username or password. The default is `Apprise API`; give each instance a different name when you host more than one.

**Use HTTPS/TLS.** Basic Auth does not encrypt credentials. HTTPS is required outside `localhost` or a trusted network.

API clients send Basic Auth with every request. Requests that explicitly accept `text/html` use a signed browser login, so **Logout** can end browser access without changing JSON or plain-text API behavior. Ordinary API calls cannot use this cookie instead of Basic Auth.
...
In the web interface, open a configuration and select its lock icon or **Authentication** in the menu. Enter a username and password, then select **Save**. Global administrators can use **Randomize** to generate both values.

The configuration ID, username, and password are three separate values. Use random values when practical and share all three with the person who needs access. The saved password is never shown again.

API clients can protect a saved configuration with `POST /auth/{KEY}`. An administrator is required to create the first login or remove one. Existing configuration users authenticate with their current login, repeat the saved username, and supply a new password. Only an administrator can change the username. `APPRISE_CONFIG_LOCK` does not block these changes. See [API Endpoints](/api/endpoints/).
...
The web interface includes **Logout**, which ends its signed browser login immediately. A browser may retain Basic Auth internally, but those cached credentials cannot silently restore an HTML login.
...
Apprise API does not require Django's browser cookie and CSRF token exchange. This keeps direct API calls from `curl`, mobile apps, and other programs simple; these clients normally omit `Origin` and continue to work as documented.
