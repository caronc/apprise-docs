...
| :--------- | :---------------------------------------------------- | :--------------------------------------------------------------------------- |
| `user`     | Requires its credentials. Tags are optional.          | Keeps the existing user access.                                              |
...
| `disabled` | Administrator only.                                   | Freezes user access while preserving configuration and credentials.          |
...
API clients manage access with `POST /auth/{KEY}`. The `access` field accepts `user`, `locked`, `public`, or `disabled` and defaults to `user` for a new record. An administrator may create public or disabled access without credentials. A configuration user may change only their password and must repeat their username. See [API Endpoints](/api/endpoints/).
