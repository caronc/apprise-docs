...
Apprise API does not require authentication by default.

Set `APPRISE_PASSWORD` and, optionally, `APPRISE_USER` to protect every endpoint with HTTP Basic Auth. A username alone disables auth with a startup warning. See [Environment Variables](/api/reference/environment/).
...
Protect a saved configuration with `POST /auth/{KEY}`. Global auth must remain configured. The first lock requires global credentials; later changes accept global or current key credentials. `APPRISE_CONFIG_LOCK` does not block these changes. See [API Endpoints](/api/endpoints/).

A key stays protected if global auth is later removed, but its lock cannot be managed until global auth returns. Keep global credentials configured while using per-key locks.
