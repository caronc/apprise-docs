...
| `/json/urls/{KEY}` | `GET`    | Returns saved URLs and tags. Each URL lists template names with their configuration defaults (or `null`). With `privacy=1`, URL secrets are hidden and all listed defaults become `null`. Server environment values are never returned. With `APPRISE_CONFIG_LOCK=yes`, global administrator credentials are required. |
