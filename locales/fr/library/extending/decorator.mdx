...
| `body_passthrough` | Non    | `True` si aucune source n'a été déclarée et que la conversion automatique a été ignorée ; sinon `False`                                             |
...
Par défaut, votre fonction reçoit le corps sans conversion automatique de son
format. Pour demander à Apprise de le convertir d'abord, ajoutez `body_format=`
à `@notify()` :
...
`body_format=` accepte un format ou un tuple de formats pris en charge. Le
premier format du tuple est utilisé par défaut. Consultez
[Conversion du Format des Messages](../message-format-conversion/) pour voir
d'autres exemples.

Sans `body_format=`, Apprise ne convertit pas le corps. Votre fonction reçoit
tout de même `body_format` et `body_passthrough` dans `kwargs`. `body_format`
indique le format fourni par l'appelant, ou `TEXT` si aucun format n'a été
précisé.

Le mode pass-through ne contourne pas le traitement normal de remise. Le corps
peut déjà avoir été découpé ou tronqué selon le mode de dépassement choisi, et
le wrapper peut encore devoir l'échapper ou l'emballer pour sa destination.
Utilisez `body_passthrough` pour éviter toute supposition sur le format source.
