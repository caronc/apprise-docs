...
Apprise inclut un ensemble de fonctions utilitaires utilisées à travers les plugins et les intégrations prises en charge.
Les auteurs de plugins devraient préférer ces helpers aux traitements ad hoc, car ils offrent un comportement cohérent, une meilleure gestion des cas limites et une journalisation plus sûre.
...
Une bonne façon de penser aux utilitaires de parsing est la suivante :

- les fonctions `parse_*()` découpent et normalisent des listes d'entrées, souvent de manière tolérante ;
- beaucoup acceptent une option `store_unparseable` afin que vous puissiez journaliser plus tard les valeurs rejetées ;
...
- `validate_regex()` est un helper strict servant à valider et éventuellement reformater une valeur avec une regex.
...
Utilisez cette fonction dans vos implémentations de `parse_url()` ou `parse_native_url()` pour convertir une URL fournie par l'utilisateur en structure normalisée. Elle gère la détection de schéma, l'échappement, l'extraction de l'utilisateur et du mot de passe, la vérification de l'hôte, le parsing du port et celui de la chaîne de requête.
...
qsd = results.get("qsd", {})  # dictionnaire de query string
```

#### Arguments clés

- `verify_host`
  Lorsque cette valeur est `True`, `parse_url()` valide l'hôte et renvoie `None` s'il est absent ou invalide. C'est un bon défaut pour des cibles réseau.
  Lorsque cette valeur est `False`, la validation de l'hôte est assouplie, ce qui est utile pour des plugins qui traitent l'hôte comme un identifiant ou n'en ont pas besoin.

- `default_schema`
  Utilisé quand l'utilisateur omet la portion `schema://`.

- `simple`
  Lorsque cette valeur est `True`, la structure renvoyée est plus petite. La plupart des plugins devraient garder `simple=False`.

- `plus_to_space`
  Contrôle si `+` dans la query string devient un espace. Apprise le laisse par défaut à `False` car `+` est fréquent dans les jetons et mots de passe.
...
## Helpers de parsing courants

### `parse_bool()`

Convertit les représentations booléennes courantes en `bool` Python. C'est la méthode recommandée pour parser des drapeaux issus d'une query string ou d'une valeur de configuration.
...
Un cas classique est une fonctionnalité **auto-activée seulement lorsqu'un autre réglage est présent**, sauf si l'utilisateur la désactive explicitement.
...
- `cast` convertit les valeurs avant parsing lorsque c'est possible ;
- `allow_whitespace` contrôle si les espaces sont traités comme séparateurs ;
- `sort` contrôle si le résultat est normalisé en liste triée unique (`True`) ou renvoyé dans l'ordre de parsing (`False`).

### `parse_emails()` et `is_email()`

Ces helpers sont couramment utilisés dans les intégrations de type email et partout où l'utilisateur peut passer plusieurs destinataires.
...
Extrait des URL depuis des chaînes ou des entrées de type liste et peut conserver les valeurs impossibles à parser afin d'aider au reporting d'erreurs.
...
`parse_call_sign()` découpe une liste d'indicatifs, puis `is_call_sign()` valide une valeur unique et renvoie `False` ou un dictionnaire.
...
- parser une liste avec `parse_call_sign(...)` ;
...
- `is_hostname(hostname, ipv4=True, ipv6=True, underscore=True)`
  Valide les noms d'hôte et éventuellement les adresses IP. Il prend en charge les underscores lorsque `underscore=True` afin de couvrir des conventions pragmatiques de Docker et d'environnements locaux.

- `is_ipaddr(addr, ipv4=True, ipv6=True)`
  Valide IPv4 et IPv6. Pour IPv6, la fonction renvoie l'adresse entourée de crochets (`[addr]`) afin de respecter les attentes de format d'URL.
...
Valide une valeur par rapport à une expression régulière. En cas de succès, renvoie la valeur nettoyée ; sinon renvoie `None`.
...
Si votre plugin définit une regex de jeton de template (comme la plupart des plugins), vous pouvez la réutiliser directement pour valider les entrées, ce qui garde la définition à un seul endroit.
