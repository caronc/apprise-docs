...
:::note
Le numéro de priorité est **uniquement un marqueur de regroupement et d'ordre de tri**.
Les services qui partagent le même nom de tag et le même numéro de priorité sont
dispatchés ensemble comme un seul groupe. Les groupes s'exécutent dans l'ordre numérique
croissant (le numéro le plus bas en premier). Les valeurs choisies n'ont pas d'importance
en elles-mêmes -- `2`, `50` et `9999` sont aussi valides que `1`, `2` et `3`.
...
3. Si **chaque** service de ce groupe réussit, le résultat est considéré comme vrai immédiatement -- les groupes de numéros supérieurs ne sont jamais déclenchés.
...
:::note
Si aucun service ne correspond à la priorité exacte spécifiée, **rien n'est envoyé**. Le résultat
retourné par `notify()` est considéré comme faux (son `status` vaut `NOMATCH` -- voir
[Résultats de notification](/library/results/)), et la CLI se termine avec le code `3` (aucun
service correspondant trouvé). C'est intentionnel -- un filtre exclusif est une sélection précise,
pas une demande de secours.
:::

### Comportement en cas d'échec de chaîne

Quand `notify()` exécute plusieurs chaînes OR indépendantes en parallèle, le comportement par défaut est de laisser **chaque chaîne se terminer** avant de retourner -- même si une chaîne a déjà échoué. Cela garantit que chaque service configuré obtient au moins une tentative.

Pour arrêter immédiatement tous les rounds d'escalade restants dès qu'une chaîne épuise ses groupes de priorité sans succès, définissez `abort_on_chain_failure=True` sur votre `AppriseAsset` :
...
# Si la chaîne 'ops' échoue complètement, Apprise s'arrête immédiatement
# et ne lance pas de nouveaux rounds d'escalade pour la chaîne 'securite'.
apobj.notify(body="Alerte critique", tag=["ops", "securite"])
```

| `abort_on_chain_failure` | Comportement en cas d'échec d'une chaîne                                         |
| :----------------------- | :------------------------------------------------------------------------------- |
| `False` (défaut)         | Toutes les chaînes se terminent ; chaque service a une chance d'être tenté       |
| `True`                   | Les rounds restants sont ignorés ; le résultat est considéré comme faux aussitôt |
...
Chaque service dispose d'un budget de temps -- s'il (avec toutes ses tentatives et pauses)
n'a pas terminé dans ce délai, Apprise cesse d'attendre plutôt que de rester bloqué
indéfiniment. Ceci est indépendant de `retry`/`wait` ci-dessus : un service peut avoir
droit à de nombreuses tentatives tout en étant coupé si l'ensemble prend trop de temps.

Par défaut, chaque service dispose de **60 secondes**. Définissez votre propre valeur avec
`service_timeout` sur `AppriseAsset`, ou désactivez complètement la limite avec `0` :

```python
from apprise import Apprise, AppriseAsset

# Donne 10 secondes à chaque service au lieu des 60 secondes par défaut
asset = AppriseAsset(service_timeout=10)
apobj = Apprise(asset=asset)
apobj.add("slack://tokenA/tokenB/tokenC")

apobj.notify(body="Vérification rapide")
```

```bash
# Équivalent CLI -- --service-limit (-SL) applique la même limite à chaque service
apprise --service-limit 10 --body "Vérification rapide" \
    "slack://tokenA/tokenB/tokenC"
```

Un service qui dépasse son délai est signalé avec un statut `TIMEOUT` plutôt qu'un
simple échec -- voir [Résultats de notification](/library/results/) pour savoir comment
vérifier cela depuis Python.

:::caution
Apprise peut arrêter _d'attendre_ un service quand son budget de temps est dépassé,
mais il ne peut pas couper de force un appel déjà en cours. Si une requête réseau est
bloquée, Python ne peut pas simplement l'interrompre au milieu.

En pratique, le service concerné peut donc continuer en arrière-plan après qu'Apprise
a signalé `TIMEOUT` et est passé à la suite. Avec un seul service synchrone lancé tout
seul, il peut même finir après son délai, car il n'y avait aucun autre travail où
Apprise pouvait vérifier la limite entre deux étapes. Pensez à `service_timeout` et
`timeout=` comme une promesse "Apprise ne restera pas à attendre plus longtemps que
ça", pas comme un bouton d'arrêt forcé du service lui-même.
:::

| Paramètre         | Type            | Contraintes                           | Par défaut |
| :---------------- | :-------------- | :------------------------------------ | :--------- |
| `service_timeout` | `int` / `float` | `0` ou plus ; `0` désactive la limite | `60`       |

---

## Services Optionnels

Le paramètre `optional` indique qu'un service peut échouer sans faire échouer toute
la notification. Lorsque `optional=yes` est activé sur un service, un échec de
livraison pour ce service n'affecte pas le résultat global. Le résultat global est
quand même considéré comme vrai si tous les services requis ont réussi.

Utilisez ce paramètre pour les points de terminaison qui doivent recevoir la
notification quand ils sont disponibles, sans faire échouer l'application appelante
quand ils ne le sont pas.

### Exemple : écrans d'affichage domestiques
...
```python
# Le résultat reste vrai même si tous les écrans sont éteints ou inaccessibles.
...
| :----------------------------------------------------------------- | :--------------- |
| Tous requis, tous réussissent                                      | Vrai (`SUCCESS`) |
| Tous requis, un ou plusieurs échouent                              | Faux (`FAILURE`) |
| Tous requis, délai dépassé sans échec confirmé                     | Faux (`TIMEOUT`) |
| Tous optionnels, tous échouent                                     | Vrai (`SUCCESS`) |
| Tous optionnels, tous dépassent le délai                           | Vrai (`SUCCESS`) |
| Tous optionnels, tous réussissent                                  | Vrai (`SUCCESS`) |
| Mélange requis/optionnels ; tous les requis réussissent            | Vrai (`SUCCESS`) |
| Mélange requis/optionnels ; au moins un requis échoue              | Faux (`FAILURE`) |
| Mélange requis/optionnels ; délai dépassé sur les requis seulement | Faux (`TIMEOUT`) |

En résumé : les échecs et dépassements de délai des services optionnels ne font
pas échouer le résultat global. Seul l'échec ou le dépassement de délai d'un
service requis peut rendre le résultat faux.
...
```python
# Le résultat reste considéré comme vrai, quel que soit le nombre d'écrans ayant répondu.
...
Si vous souhaitez que **chaque** service d'une session Apprise partage les mêmes valeurs par défaut de tentatives, d'attente et de délai sans modifier chaque URL, définissez-les sur `AppriseAsset` :
...
asset = AppriseAsset(
    ...
    service_timeout=10,        # abandonner un service bloqué après 10 secondes
)
...
apobj.notify(body="Valeurs par défaut appliquées à chaque service")
```
...
## Remplacement du Délai par Appel

Passez `timeout=` à `notify()` pour limiter la durée de **cet appel précis**, du début à
la fin -- pour tous les services qu'il notifie. Contrairement à `service_timeout` (un
budget par service défini sur `AppriseAsset`), `timeout` est un plafond pour l'appel
entier.

```python
# Limite cet appel entier à 30 secondes, quel que soit le service_timeout
# propre à l'asset
apobj.notify(body="Alerte urgente", timeout=30)
```

```bash
# Équivalent CLI -- --limit (-L) s'applique à l'ensemble de l'exécution
apprise --limit 30 --body "Alerte urgente" \
    "slack://tokenA/tokenB/tokenC"
```

Lorsque les deux sont définis, **le délai le plus court l'emporte** pour chaque service --
`timeout=` ne prolonge jamais un `service_timeout` plus strict, et un `service_timeout`
généreux ne remplace jamais un `timeout=` plus serré.

```python
from apprise import Apprise, AppriseAsset

# Chaque service dispose normalement de 60 secondes (valeur par défaut
# d'AppriseAsset)...
apobj = Apprise(asset=AppriseAsset())
apobj.add("slack://tokenA/tokenB/tokenC")

# ...mais cet appel coupe chaque service à 5 secondes à la place.
apobj.notify(body="Abandonner rapidement cette fois", timeout=5)
```

`timeout` vaut `0` par défaut, ce qui signifie "aucun plafond au niveau de l'appel" --
seul le `service_timeout` de `AppriseAsset` s'applique. Les valeurs négatives déclenchent
une erreur, tout comme un `service_timeout` invalide.
...
avant que le résultat soit considéré comme vrai. Un suffixe `:N` sur un token ne
...
Les exemples ci-dessous illustrent des schémas de notification courants.

:::note
Les numéros de priorité sont des **marqueurs d'ordre relatif**, pas des valeurs fixes.
Un service à la priorité `2` sera toujours envoyé avant un service à la priorité `500`
pour le même tag -- les valeurs exactes n'ont pas d'importance, seul leur ordre compte.
:::
...
numéros sont arbitraires ; seul l'ordre importe.

<Tabs syncKey="config-format">
<TabItem label="TEXT">

```apache
# Premier essai (numéro inférieur = urgence supérieure)
2:alertes=slack://tokenA/tokenB/tokenC

# Secours ; s'exécute uniquement si la priorité 2 échoue
...
```python
# Chaîne d'escalade : Slack d'abord, e-mail seulement en cas d'échec
...
**Trois chaînes OR avec plusieurs niveaux de priorité** -- `alpha`, `beta` et `gamma`
sont des chaînes indépendantes dispatchées simultanément. Au sein de chaque chaîne,
les groupes de priorité inférieure s'exécutent en premier ; les échecs escaladent au
niveau suivant. Par défaut, chaque chaîne s'exécute jusqu'à son terme même après qu'une
autre chaîne a déjà épuisé tous ses niveaux sans succès -- voir « Comportement en cas
d'échec de chaîne » plus haut pour changer cela.

<Tabs syncKey="config-format">
<TabItem label="TEXT">

```apache
# alpha : trois niveaux avec des valeurs de priorité arbitraires
...
# beta : un seul niveau à la priorité 500
500:beta=discord://webhook_id2/webhook_token2

# gamma : deux niveaux ; sans préfixe = priorité 0 (la plus haute)
...
urls:
  ...
  # gamma -- deux niveaux (sans préfixe = priorité 0)
...
**Ordre de dispatch quand les trois chaînes sont combinées en OR :**

| Round                              | alpha                      | beta                   | gamma                     |
| :--------------------------------- | :------------------------- | :--------------------- | :------------------------ |
| 1                                  | Slack + Gmail (priorité 1) | Discord (priorité 500) | Telegram 2 (priorité 0)   |
| 2 (si échec au round 1)            | Telegram (priorité 5)      | -- épuisé --           | Telegram 3 (priorité 200) |
| 3 (si échec au round 2 pour alpha) | Discord (priorité 700)     |                        |                           |
...
**Deux chaînes OR indépendantes avec surcharge de tentatives par appel** -- chaque
...
apobj.notify(body="Alerte équipe", tag=["devops:3", "direction:2"])
```
...
**Filtre AND** -- seuls les services portant **tous** les tags listés sont dispatchés.

<Tabs syncKey="config-format">
<TabItem label="TEXT">

```apache
# Les deux tags sur la même URL ; seul ce service correspond au filtre AND
...
apobj.notify(body="Alerte combinée", tag=[["devops", "direction"]])
```

```bash
# AND -- valeurs séparées par une virgule dans un seul flag --tag
apprise --config=config.yml --tag="devops, direction" --body="Alerte combinée"
```
...
**Dispatch exclusif** -- un préfixe de priorité dans le filtre cible uniquement ce
niveau exact. Aucune chaîne d'escalade n'est construite ; les services correspon-
dants sont dispatchés en un seul lot plat.
...
```python
# Exclusif : seules les entrées de priorité 50 s'exécutent ; Slack est ignoré
...
| Délai par service                           | `AppriseAsset(service_timeout=N)` ou `--service-limit N`                | Par définition de service       |
...
| Remplacement du délai                       | `notify(timeout=N)` ou `--limit N`                                      | Un appel notify() / CLI         |
...
| Arrêt anticipé sur échec de chaîne          | `AppriseAsset(abort_on_chain_failure=True)`                             | Tous les services de la session |
