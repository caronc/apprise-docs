...
### `send()` Peut Être Appelé Plusieurs Fois

Un long message peut être réparti sur plusieurs appels à `send()`. Apprise ne
transmet `attach` qu'à un seul de ces appels. Le test `if attach:` évite donc
d'envoyer les mêmes fichiers avec chaque partie du message.

Par défaut, la pièce jointe accompagne la première partie. Pour l'envoyer avec
une autre partie, surchargez `_attachment_send_index()`. Consultez
[Connaître Sa Position Parmi Plusieurs Appels à `send()`](../extending/message-format-conversion/#connaitre-sa-position-parmi-plusieurs-appels-a-send)
...
### Préférez `attachment.open()` pour les API de Téléversement
...
## Exemples Pratiques dans les Plugins Principaux
