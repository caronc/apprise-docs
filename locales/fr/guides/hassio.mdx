...
Cette méthode est idéale si vous n'avez qu'un **seul** service de notification ou si vous souhaitez démarrer rapidement sans créer de fichiers supplémentaires.

**Modifiez `configuration.yaml`**

Ajoutez l'entrée suivante à votre fichier de configuration. Remplacez l'URL par celle de votre service.
...
notify:
  - name: apprise_quick
    platform: apprise
    # Définissez directement ici l'URL Apprise
...
:::note
La directive `include` ne fonctionne que si votre serveur Apprise API n'a pas `APPRISE_CONFIG_LOCK` défini à `yes`.
:::
...
Si votre serveur Apprise API fonctionne avec `APPRISE_CONFIG_LOCK` activé, cette méthode ne fonctionnera pas.
:::
