...
La CLI est l’une des portes d’entrée d’Apprise. Elle utilise les mêmes URL de notification que le reste, et peut s’adresser à un serveur API Apprise plutôt que d’envoyer directement.
...
<picture class="apprise-flow apprise-flow--cli">
  ...
  <img
    ...
    alt="La CLI Apprise mise en évidence parmi les façons d’envoyer des notifications."
    loading="lazy"
  />
</picture>
...
La plupart des utilisateurs l'installent via `pip install apprise`. Si vous utilisez [uv](https://docs.astral.sh/uv/), `uv tool install apprise` fait la même chose.
