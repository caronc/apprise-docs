...
Il arrive qu'un service réponde lentement, ou soit simplement injoignable. Ces options limitent la durée pendant laquelle Apprise est prêt à attendre avant d'abandonner et de passer à la suite.

| Flag  | Option longue     | Description                                                                                                                                                      |
| :---- | :---------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `-L`  | `--limit`         | Abandonne l'exécution entière si elle prend trop de temps, en secondes (par ex. `2.5`). Par défaut `0` -- aucune limite.                                         |
| `-SL` | `--service-limit` | Abandonne un service s'il prend trop de temps, en secondes, ou `0` pour désactiver complètement. Laissez vide pour la valeur par défaut d'Apprise (60 secondes). |

```bash
# Abandonner l'exécution entière après 10 secondes
apprise -t "Statut" -b "Vérification rapide" --limit 10 \
    "slack://tokenA/tokenB/tokenC"

# N'accorder que 5 secondes à chaque service, quel que soit leur nombre
apprise -t "Statut" -b "Vérification rapide" --service-limit 5 \
    "slack://tokenA/tokenB/tokenC" "discord://webhook_id/webhook_token"
```

:::tip
`--limit` plafonne l'exécution entière ; `--service-limit` plafonne chaque service individuellement. Utilisez `--service-limit` pour une patience homogène sur de nombreux services ; utilisez `--limit` quand vous voulez simplement que la commande entière se termine avant une certaine durée.
:::

:::caution
Une limite de temps peut empêcher Apprise de continuer à _attendre_ un service, mais
elle ne peut pas couper de force un appel déjà en cours. Python ne sait pas arrêter
proprement une requête réseau bloquée en plein milieu.

Avec la CLI, si le résultat global est `TIMEOUT`, Apprise attend encore environ
5 secondes pour laisser l'appel abandonné se terminer tout seul. S'il tourne
toujours, la commande se termine quand même en arrêtant le processus. En pratique,
cela veut dire qu'une commande CLI finit en général dans environ
`--limit`/`--service-limit` + 5 secondes. C'est utile pour un script cron ou une
automatisation qui ne doit pas rester bloquée indéfiniment.

Cette garantie concerne la CLI uniquement. Une application Python qui utilise
Apprise comme bibliothèque ne peut pas arrêter tout son propre processus juste
pour stopper une seule notification.
:::
...
| `-tv` | `--template-var`  | Fournir une valeur pour un `${NOM}` utilisé par une configuration YAML, sous la forme `NOM=VALEUR`. Répétable, mais une seule fois par nom. |
...
| `-h`  | `--help`          | Afficher le message d'aide puis quitter.                                                                                                    |

## Exemple de variable de substitution

Commencez par déclarer la valeur dans une configuration YAML :
...
Fournissez-la ensuite lors de l'envoi :
...
Seuls les noms déclarés sous `template:` sont remplacés. Consultez
[Variables de substitution](../getting-started/template/) pour les valeurs par
défaut, l'environnement, les règles de placement et d'autres exemples.
...
| `APPRISE_TEMPLATE_<NOM>`                | Fournir une valeur pour un `${NOM}` utilisé par une configuration YAML. Par exemple, `APPRISE_TEMPLATE_API_KEY` remplit `${API_KEY}`. Voir [Variables de substitution](../getting-started/template/).                                                                                                                        |
| `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY` | Variables de proxy standard prises en compte automatiquement (non spécifiques à Apprise). Acheminez les notifications sortantes via un proxy, ou exemptez certains hôtes avec `NO_PROXY`. Voir [Prise en charge des proxys](/library/advanced/#prise-en-charge-des-proxys) pour plus de détails, y compris les proxys SOCKS. |
...
Le plugin utilise la version 2 par défaut. Il envoie la clé de configuration dans `X-Apprise-Config-ID`, ce qui la retire du chemin de la requête HTTP. Ajoutez `?v=1` pour un ancien serveur Apprise API qui exige `/notify/{KEY}`.
...
<Tabs>
<TabItem label="Sans Authentification">
...
<TabItem label="Avec Authentification">

```bash
# Les identifiants de l'URL sont envoyés avec Basic Auth.
apprise -t "Job Finished" \
    "apprises://user:password@apprise.example.com/my-alerts"

# Une connexion administrateur avec mot de passe uniquement laisse le nom vide.
apprise -t "Job Finished" \
...
- `0` si tous les services notifiés ont réussi.
- `1` si tous les services notifiés ont échoué, ou si la notification n'a pas pu démarrer.
...
- `4` si certains services ont envoyé la notification avec succès et qu'au moins un autre ne l'a pas fait.
- `5` si aucun service n'a envoyé la notification avec succès, mais que le seul problème était le manque de temps (voir [Délais](#délais) ci-dessus).
