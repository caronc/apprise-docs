...
| `/json/urls/{KEY}` | `GET`    | Renvoie les URL et les tags enregistrés. Chaque URL indique aussi les noms de substitution qu'elle utilise et lesquels doivent être fournis. Avec `privacy=1`, les secrets des URL et les valeurs par défaut de la configuration sont masqués. Les valeurs de l'environnement du serveur ne sont jamais renvoyées. Avec `APPRISE_CONFIG_LOCK=yes`, les identifiants de l'administrateur global sont requis. |
