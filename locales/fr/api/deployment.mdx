---
title: Déploiement
description: How to deploy and operate the API Apprise using containers, Docker Compose, or Kubernetes.
sidebar:
  order: 2
---
...
The **API Apprise** is designed to run as a containerized service.  
...
API Apprise is a **stateless-first** service with **optional persistent state**.
...
Choose one deployment method below to get API Apprise running quickly.

<Tabs>
  <TabItem label="Docker">```bash
...
<TabItem label="Kubernetes">
  <Tabs>
<TabItem label="Minimal Example">```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: apprise
...
For a more security conscious setup, you can run API Apprise as a non-root user with a read only root filesystem and explicit ephemeral volumes.
...
API Apprise supports persistent storage for:
...
For public or multi-tenant environments, API Apprise supports hardened execution.
...
### Kubernetes Hardening Remarques
...
API Apprise exposes a health endpoint:
...
If API Apprise is hosted behind a reverse proxy or served under a subpath, set `APPRISE_BASE_URL` to accommodate this.
...
API Apprise does **not** implement authentication internally.
