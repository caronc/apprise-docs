...
| `403` | Forbidden                       | Le paramètre du serveur ou le niveau d'accès actuel refuse la requête. Par exemple : stockage avec état désactivé, liste `/cfg` indisponible ou contenu de configuration verrouillé.                                                                                                            |
...
| `429` | Too Many Requests               | Le proxy inverse a temporairement limité la requête. Attendez la valeur `Retry-After` avant de réessayer.                                                                                                                                                                                       |
