...
| `403` | Forbidden                       | Le serveur est configuré pour refuser la requête quels que soient les identifiants fournis (par exemple `APPRISE_CONFIG_LOCK=yes`, le listing `/cfg` désactivé, ou `APPRISE_STATELESS_MODE=disabled`).                                                                                          |
...
| `429` | Too Many Requests               | Trop d'échecs Basic récents pour ce client et cette clé. Attendez la valeur `Retry-After` avant de réessayer.                                                                                                                                                                                   |
