...
| format              | Non    | Format du message : **text**, **html** ou **markdown**. La valeur par défaut est **text**.                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| e2ee                | Non    | Contrôle le chiffrement Matrix Olm/MegOLM. La valeur par défaut est **yes**. Apprise chiffre les messages et les pièces jointes des salons chiffrés et active le chiffrement pour les nouveaux salons qu’il crée. Nécessite le paquet `cryptography` et **matrixs://** ; le mode webhook n’est pas pris en charge. Lorsqu’il est disponible, le surcoût E2EE est inclus dans le calcul de la taille. Définissez **no** pour toujours envoyer en clair et éviter la création de salons E2EE. |
