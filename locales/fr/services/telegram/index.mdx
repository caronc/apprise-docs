...
| format    | Non         | La valeur par défaut est _html_. Définissez-la sur _markdown_ lorsque vous voulez utiliser le traitement Markdown de Telegram.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| album     | Non         | Regroupe les photos et les vidéos dans des albums Telegram avec la valeur `yes`. Les GIF, les fichiers audio et les documents sont envoyés séparément dans leur ordre d'origine. Si Telegram refuse un album, chaque élément est envoyé séparément. Une photo ou une vidéo encore refusée est alors envoyée comme un fichier ordinaire. La valeur par défaut est `no`.                                                                                                                                                                                                      |
...
:::note
Telegram accepte les pièces jointes jusqu'à 50 Mo, et les photos jusqu'à 10 Mo. Une photo plus volumineuse est envoyée comme un fichier ordinaire. Un fichier de plus de 50 Mo ne peut pas être envoyé, et la notification est signalée en échec.
:::
...
Vous avez plusieurs photos à envoyer ? Regroupez-les dans un seul album Telegram :

```bash
# Supposons que notre {bot_token} soit 123456789:abcdefg_hijklmnop
# Supposons que le {chat_id} de lead2gold soit 12315544
# Les photos arrivent dans un seul album au lieu de trois messages séparés
apprise -vv -t "Titre du Message de Test" -b "Corps du Message de Test" \
   --attach=/chemin/vers/une.jpg \
   --attach=/chemin/vers/deux.jpg \
   --attach=/chemin/vers/trois.jpg \
   tgram://123456789:abcdefg_hijklmnop/12315544/?album=yes
```
