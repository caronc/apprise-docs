...
Le formatage sur l'Apprise-API est entièrement optionnel, requête par requête. Un opérateur qui souhaite que chaque requête sur son serveur suppose un format sans que chaque appelant ait à le préciser peut définir la variable d'environnement `APPRISE_DEFAULT_FORMAT` (par exemple `APPRISE_DEFAULT_FORMAT=text`) -- consultez la [référence des variables d'environnement](../../api/reference/environment/) pour plus de détails. Une valeur `format` fournie dans la requête a toujours priorité sur cette valeur par défaut du serveur.
