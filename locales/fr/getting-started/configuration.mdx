...
Utilisez le placeholder `_` lorsque l'URL ne doit contenir aucun identifiant :
...
:::note
Dans les blocs de tokens YAML, écrivez toujours `password:` -- pas `pass:`. Le raccourci `pass` est une convention du chemin d'URL (`user:pass@host`) résolue par l'analyseur d'URL. Les clés de tokens YAML correspondent directement aux noms de paramètres du plugin, dont le nom correct est `password`.
:::
