...
L'un des avantages les plus pratiques de YAML par rapport au TEXT est que **les valeurs des paramètres n'ont jamais besoin d'être encodées en URL**. Dans une URL brute, toute valeur contenant des caractères spéciaux — espaces, `@`, `#`, chevrons — doit être encodée en pourcentage avant de pouvoir figurer dans une chaîne de requête. En YAML, ces valeurs sont des chaînes ordinaires sans encodage nécessaire.
...
```yaml
urls:
  - mailtos://joe:myP%40ss%23w0rd%21@example.com:
...
:::note
La partie `schema://user:pass@host` de l'URL reste une syntaxe URL standard, donc les identifiants qui y sont intégrés doivent toujours être encodés en URL lorsqu'ils contiennent des caractères réservés. Seules les **sous-clés** (paramètres extraits de la chaîne de requête) sont exemptées de cette contrainte.
:::
