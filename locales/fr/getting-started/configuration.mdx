...
### Deux Styles d'Indentation Equivalents

Les sous-clés de tokens peuvent être écrites au même niveau d'indentation que la clé d'URL (**forme voisine**) ou indentées d'un niveau supplémentaire (**forme enfant**). Les deux formes produisent des résultats identiques.

**Forme enfant** -- tokens indentés d'un niveau sous la clé d'URL (imbrication YAML standard) :
...
**Forme voisine** -- tokens au même niveau d'indentation que la clé d'URL :
...
Les deux produisent exactement le même service chargé. La forme voisine est particulièrement pratique lorsqu'on copie une configuration depuis des outils ou des modèles qui utilisent une indentation plate.

Le placeholder `_` est utile lorsque l'URL ne doit contenir aucun identifiant -- tous les paramètres sont fournis via les tokens :
...
:::note
Dans les blocs de tokens YAML, écrivez toujours `password:` -- pas `pass:`. Le raccourci `pass` est une convention du chemin d'URL (`user:pass@host`) résolue par l'analyseur d'URL lors du traitement de la chaîne d'URL. Les clés de tokens YAML correspondent directement aux noms de paramètres du plugin, dont le nom correct est `password`.
:::
