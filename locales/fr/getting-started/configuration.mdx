...
Les sous-clés YAML sont fusionnées avec les valeurs extraites de l'URL avant la création du service. Cela signifie que des parties courantes de l'URL comme `user`, `password`, `host`, `port`, `verify` et `tag` peuvent être fournies comme clés YAML, tout comme les paramètres propres au service listés pour ce plugin.

Tous les paramètres listés dans le tableau `## Détail des Paramètres` d'un service peuvent être utilisés comme sous-clés YAML. Le [service Email](/services/email/) est illustré ci-dessus, mais la même approche fonctionne pour n'importe quel service Apprise.

Vous pouvez toujours conserver des valeurs dans l'URL lorsque c'est plus pratique. Les paramètres de requête présents dans l'URL, comme `?user=abc&password=123`, sont analysés comme des remplacements URL pour les champs qu'ils représentent.

Le même modèle est utile pour d'autres services. Par exemple, une configuration XMPP peut garder une URL courte et placer les identifiants ainsi que le comportement de connexion dans YAML :
