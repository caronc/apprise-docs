...
Les composants courants d'une URL, comme `user`, `password`, `host`, `port` et `verify`, peuvent être fournis comme sous-clés. Il en va de même pour les paramètres propres au service qui figurent dans le tableau `## Détail des Paramètres` du plugin, y compris les éléments nommés de l'URL comme le `botname:` de Discord.

:::note
Certains raccourcis représentent plusieurs valeurs d'URL. Par exemple, le `token:` de Slack peut représenter un jeton de bot ou trois jetons de webhook. Conservez ces raccourcis dans l'URL ; utilisés comme sous-clés, ils sont ignorés et un avertissement est consigné dans le journal.
:::
...
La langue utilisée par Apprise pour ses propres textes peut être définie sous `asset` via `language` (ou `lang`), par exemple `language: fr`. Omettez-la pour suivre la langue de l'ordinateur sur lequel Apprise s'exécute.
...
Une configuration YAML peut déclarer des valeurs `${NOM}` à fournir plus tard :
...
Seuls les noms déclarés sous `template:` sont remplacés. Une variable ne peut
définir ni `tag:`, ni `tags:`, ni le nom du service avant `://`. Consultez
[Variables de substitution](./template/) pour les règles et les exemples de la
CLI, de l'API et de l'environnement.
