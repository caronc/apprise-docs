...
description: "Depannage high memory, RAM, and CPU usage in API Apprise Docker containers. Learn how to reduce resource consumption for low-traffic deployments."
sidebar:
...
**Is the API Apprise Docker container using more RAM or memory than you expect?**
...
If you only use a small set of notification services, you can reclaim additional memory by telling the API which services you actually need. The API Apprise will evict the optional libraries used exclusively by the disabled plugins from memory at startup.
